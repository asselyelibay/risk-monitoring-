...
st.markdown('<p class="main-header">🛡️ Anti-AI Bubble Portfolio</p>', unsafe_allow_html=True)
...
if min_vol > target_vol:
    st.warning(f"""
    ⚠️ Target volatility ({target_vol*100:.1f}%) is below the minimum achievable volatility 
...
if not result.success:
    st.error("⚠️ Portfolio optimization failed due to constraints.")
    st.stop()
...
with col4:
    st.metric("⚠️ Max Drawdown", f"{max_dd*100:.2f}%")
with col5:
